...
const card = () => {
  class Card {
    constructor(src, alt, title, descr, price, curs) {
      this.src = src, this.alt = alt, this.title = title, this.descr = descr, this.curs = curs, this.wrapper = document.querySelector('.menu__field >.container'), this.price = price * this.curs;
    }
  ...
  getCard('http://localhost:3000/cardInfoGet').then(data => {
    data.forEach(_ref => {
      let {
        src,
        alt,
...
const form = () => {
  ...
  function bindForm(selector) {
    selector.addEventListener('submit', e => {
      ...
      const formData = new FormData(selector);
      const json = JSON.stringify(Object.fromEntries(formData.entries()));
      postData('http://localhost:3000/postInfo', json).then(() => {
        setTimeout(closeModal, 500), setTimeout(() => {
          sanksWrapper.style.display = 'block';
        }, 1000), setTimeout(() => {
          sanksWrapper.style.display = 'none';
        }, 4000);
      }).catch(() => {
        closeModal(), modalError.style.display = 'block', setTimeout(() => {
          modalError.style.display = 'none';
        }, 4000);
      }).finally(selector.reset());
    });
  }
...
const slider = () => {
  ...
  slidesField.style.display = 'flex', slidesField.style.width = 100 * slides.length + '%', slidesField.style.transition = '0.7s all', wrapperSlides.style.overflow = 'hidden';
  next.addEventListener('click', () => {
    if (offset == +width.replace(/\D/gi, '') * (slides.length - 1)) {
      offset = 0;
    } else {
